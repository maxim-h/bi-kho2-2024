...
path_dirs <- c(
  "../Data/3.STAR/Fixed_fresh_K562_noPEG_R1/Solo.out/GeneFull/raw",
...
for (i in 1:8) {
  ...
  count_between_knee_and_inflection_ED_yes[i] <- dim(subset(
    ED_si,
    (inflection_points[i] < UMIcount) & (UMIcount < knee_points[i])
  ))[1]
  count_between_knee_and_inflection[i] <- dim(subset(
    si,
    (inflection_points[i] < UMIcount) & (UMIcount < knee_points[i])
  ))[1]
  ...
  count_more_than_knee_ED_yes[i] <- dim(subset(
    ED_si,
    knee_points[i] < UMIcount
  ))[1]
  count_more_than_knee[i] <- dim(subset(
    si,
    knee_points[i] < UMIcount
  ))[1]
  ...
  count_between_100_and_inflection_ED_yes[i] <- dim(subset(
    ED_si,
    100 < UMIcount & UMIcount < inflection_points[i]
  ))[1]
  count_between_100_and_inflection[i] <- dim(subset(
    si,
    100 < UMIcount & UMIcount < inflection_points[i]
  ))[1]

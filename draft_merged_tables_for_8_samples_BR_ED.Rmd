...
UMI_per_CB <- function(counts_path_s1, barcodes_path_s1, genes_path_s1, counts_path_s2, barcodes_path_s2, genes_path_s2){
  ...
  DF_umi_per_CB <- as.data.frame(umi_per_CB) %>%
    ...
    DF_umi_per_CB <- DF_umi_per_CB%>%mutate(barcode = rownames(DF_umi_per_CB))
...
```{R sample UMI_counts, warning = FALSE, message = FALSE}
...
```
Проаназируем количество CB полученное для ED
```{R}

CB_ED_all_values <- unlist(ED_CB_list) # 47161

CB_ED_unique <- unique(ED_CB_list)  # 13431

CB_ED_value_counts<- table(CB_ED_all_values)

CB_ED_common <- names(CB_ED_value_counts[CB_ED_value_counts == 8]) #780

CB_ED_common_2 <- names(CB_ED_value_counts[CB_ED_value_counts >= 2]) #10337


```
Считаем "суммарный образец", сколько в нем будет будет отобранных отсечкой  BR, хороших баркодов по ED и нет
Задаем функцию 'merge_sequentially'
```{R}

merge_sequentially <- function(...){
  # Список датафреймов
  data_frames <- list(...)
  ...
  for (i in 2:length(data_frames)) {
    merged_df <- merge(merged_df, data_frames[[i]], by = "barcode", all = TRUE,
...
Получаем суммарный образец
```{R}

summ <- merge_sequentially(s1, s2, s3, s4, s5, s6, s7, s8)
summ_BR <- merge_sequentially(BR_s1, BR_s2, BR_s3, BR_s4, BR_s5, BR_s6, BR_s7, BR_s8) 
```
А теперь начинаем отбирать с помощью ED и BR
```{R}
sum_ED <-  subset(summ, (summ$barcode %in% ED_CB_list))
sum_not_ED <-  subset(summ, !(summ$barcode %in% ED_CB_list))
# в такой ситуации остаются все капли, которые прошли ED, и те, в #которых хотя бы в одном образце выше Knee_point
sum_ED_BR <- subset(sum_ED, (sum_ED$umi_per_CB > 409 |
                               sum_ED$umi_per_CB_2 > 395 |
                               sum_ED$umi_per_CB_3 > 1308 |
                               sum_ED$umi_per_CB_4 > 1143|
                               sum_ED$umi_per_CB_5 > 325 |
                               sum_ED$umi_per_CB_6 > 231|
                               sum_ED$umi_per_CB_7 > 646 |
                               sum_ED$umi_per_CB_8 > 325
                               ))
# это не равно тому, чтобы объединить все индивидуальные датафреймы #типа BR_ED_sX 
#я считала, там получается 11207, то есть примерно на #800 меньше, так как туда не #включаются те баркоды, которые прошли #проверку ED, но для этого же образца не прошли #проверку BR). #так сделать, нужно отбирать индивидуально для списка баркодов из ED по #каждому но так остаются NA


ED_s1 <-  subset(s1, (row.names(s1) %in% ED_CB_1))
ED_s2 <-  subset(s2, (row.names(s2) %in% ED_CB_2 ))
ED_s3 <-  subset(s3, (row.names(s3) %in% ED_CB_3 ))
ED_s4 <-  subset(s4, (row.names(s4) %in% ED_CB_4 ))
ED_s5 <-  subset(s5, (row.names(s5) %in% ED_CB_5 ))
ED_s6 <-  subset(s6, (row.names(s6) %in% ED_CB_6 ))
ED_s7 <-  subset(s7, (row.names(s7) %in% ED_CB_7 ))
ED_s8 <-  subset(s8, (row.names(s8) %in% ED_CB_8 ))

summ_ED_by_one <- merge_sequentially(ED_s1, ED_s2, ED_s3, ED_s4, ED_s5, ED_s6, ED_s7, ED_s8)

#можно получить, из объединенного датасета из индивидуальных отобранных по BR и ED, выбрав #оттуда баркоды. Здесь это BR_ED_summ
# в этом DF те, которые прошли фильтрацию ED в рамках одного из образцов, и количество UMI в #этом же образце выше inflection point
BR_ED_summ <- merge_sequentially(BR_ED_s1, BR_ED_s2, BR_ED_s3, BR_ED_s4, BR_ED_s5, BR_ED_s6, BR_ED_s7, BR_ED_s8)
sum_by_ind_BR_ED <- subset(sum_ED_BR, sum_ED_BR$barcode %in% BR_ED_summ$barcode) 

# можно посмотреть на разность # 860 баркодов
diff_ED_BR_summ <- subset(sum_ED_BR, !(sum_ED_BR$barcode %in% sum_by_ind_BR_ED$barcode))



```

   Теперь смотрим на пустые капли. Отбираем те, что выше отсечки по BR inflection, для каждого она своя.
````{R}
# так получается 656
sum_not_ED_BR <- subset(sum_not_ED, (sum_not_ED$umi_per_CB > 409 |
                               sum_not_ED$umi_per_CB_2 > 395 |
                               sum_not_ED$umi_per_CB_3 > 1308 |
                               sum_not_ED$umi_per_CB_4 > 1143|
                               sum_not_ED$umi_per_CB_5 > 325 |
                               sum_not_ED$umi_per_CB_6 > 231|
                               sum_not_ED$umi_per_CB_7 > 646 |
                               sum_not_ED$umi_per_CB_8 > 325
                               ))
# а если отобрать аккуратно по одному образцу, из неурезанных, с помощью BR файлов:
no_ED_s1 <-  subset(s1, !(row.names(s1) %in% ED_CB_1))
no_ED_s2 <-  subset(s2, !(row.names(s2) %in% ED_CB_2 ))
no_ED_s3 <-  subset(s3, !(row.names(s3) %in% ED_CB_3 ))
no_ED_s4 <-  subset(s4, !(row.names(s4) %in% ED_CB_4 ))
no_ED_s5 <-  subset(s5, !(row.names(s5) %in% ED_CB_5 ))
no_ED_s6 <-  subset(s6, !(row.names(s6) %in% ED_CB_6 ))
no_ED_s7 <-  subset(s7, !(row.names(s7) %in% ED_CB_7 ))
no_ED_s8 <-  subset(s8, !(row.names(s8) %in% ED_CB_8 ))
summ_notED_by_one <- merge_sequentially(no_ED_s1, no_ED_s2, no_ED_s3, no_ED_s4, no_ED_s5, no_ED_s6, no_ED_s7, no_ED_s8)
#теперь нам надо выбрать по BR

#можно получить, из объединенного датасета из индивидуальных отобранных по BR и  пустых по #ED, выбрав оттуда баркоды. Здесь это BR_noED_summ
# в этом DF те, которые прошли фильтрацию ED в рамках одного из образцов, и количество UMI в #этом же образце выше inflection point
BR_noED_summ <- merge_sequentially(BR_noED_s1, BR_noED_s2, BR_noED_s3, BR_noED_s4, BR_noED_s5, BR_noED_s6, BR_noED_s7, BR_noED_s8)
sum_by_ind_BR_noED <- subset(summ_notED_by_one, summ_notED_by_one$barcode %in% BR_noED_summ$barcode) 


````

Если считать первым способом, то есть собрать все капли хоть раз(хоть в одном образце) названные "не пустыми", и хоть в одном образце выше BR inflection отсечки, то можно работать с этими данными:

```{R}

sum_not_ED_BR %>% write.csv("sum_8_not_ED_BR.csv")
sum_ED_BR %>% write.csv("sum_8_ED_BR.csv")
```

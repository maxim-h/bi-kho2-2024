...
Посчитаем какая доля баркодов отбирается в интервале между knee point и inflection point для каждого образца

```{R}
count_between_knee_inflection <- c()
count_between_knee_inflection_ED <- c()
prop_between_knee_inflection <- c()

for (i in 1:8) {
  # Получение данных для i-го образца
  ED_si <- subset(get(paste0("s", i)), row.names(get(paste0("s", i))) %in% get(paste0("ED_CB_", i)))
  si <- get(paste0("s", i))
  between_knee_and_inflection_ED_si <- subset(ED_si, 
                                               inflection_list[i] < umi_per_CB & umi_per_CB < knee_list[i])
  between_knee_and_inflection_si <- subset(si, 
                                            inflection_list[i] < umi_per_CB & umi_per_CB < knee_list[i])
  
  # Подсчет и пропорция
  count_between_knee_inflection_ED[i] <- dim(between_knee_and_inflection_ED_si)[1]
  count_between_knee_inflection[i] <- dim(between_knee_and_inflection_si)[1]
  prop_between_knee_inflection[i] <- count_between_knee_inflection_ED[i] / dim(between_knee_and_inflection_si)[1]
}
print(count_between_knee_inflection)
print(count_between_knee_inflection_ED)
print(round(prop_between_knee_inflection, 4))

```
Видим, что отбирается очень по-разному
